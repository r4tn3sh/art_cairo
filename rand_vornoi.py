...
for loop1 in range(0,noofcenters):
    for loop2 in range(0, noofcenters):
        ...
        for loop3 in range(0, 4):
            if not ((loop1==temp_idx and loop2!=temp_idx) or (loop2==temp_idx and loop1!=temp_idx)):
                continue
            if loop3==0: #y=0
...
for loop1 in range(0,noofcenters):
#for loop1 in range(temp_idx,temp_idx+1):
    idx = 0
    for loop2 in range(0, noofcenters):
        if (loop2==loop1):
            continue
    ...
    for loop2 in range(0, noofcenters):
        ...
        for loop3 in range(0,8):
            ...
            for loop4 in range(0,noofcenters):
                ...
                len_b=veclen(candx,candy,base_x[loop1],base_y[loop1])
                len_c=veclen(candx,candy,tempx,tempy)
                if (len_a>len_b and len_a>len_c and len_c>0.0001):
                    cand_flag=0
            if cand_flag==1:
                ax[loop1][idx] = tempx
                ay[loop1][idx] = tempy
...
#for loop1 in range(temp_idx,temp_idx+1):
    ...
    print(ax[loop1])
    print(ay[loop1])
    draw_spiral(ax[loop1], ay[loop1], 0, Nidx[loop1], delta, 0.05, 150)
